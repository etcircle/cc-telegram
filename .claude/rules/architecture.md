...
Handler modules (handlers/):
  ...
  updater.py          ─ /update orchestration (owner-only manual command, no
                        ...
                        and both no-relaunch exits QUARANTINE the window
                        (tmux_manager registry): send_to_window re-checks
                        pane_current_command before typing into it — shell/
                        query-failure refuses with an in-topic "NOT delivered"
                        error, a live Claude clears it (see the /update
                        design bullet). Module-level
  ...
  late_answer.py      ─ Wave A AFK auto-resolve adaptation (leaf; stdlib +
                        ...
                        **AskUserQuestion**(…) summary prefix — safe false
                        ...
                        route-ordering delivery subsequence with the PRE-SEND
                        set_route_user_turn_at stamp, failure re-attaches the
                        original keyboard).
