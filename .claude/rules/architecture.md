...
Handler modules (handlers/):
  ...
  updater.py          ─ /update orchestration (owner-only manual command, no
                        ...
                        window) restarts ONLY that one window (the SCOPED default,
                        /update in the invoking topic — bot.update_command
                        resolves the binding + passes it; unbound → error, no
                        run). Scoped renders _format_scoped_summary (one line);
                        fleet renders _format_summary. The single-flight guard
                        covers both. Idle gate is TWO-factor: route_is_idle =
