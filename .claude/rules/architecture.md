...
Additional modules:
  ...
  delivery.py                 ─ GH #50 delivery-result + payload-shaping leaf (pure
                                ...
                                RAW-CONTROL-BYTE refusal — `-l` stops TMUX
                                interpreting key NAMES but passes C0/ESC to the
                                pty VERBATIM, rig-confirmed, so an embedded
                                ESC[B + digit is a cursor-move + HOTKEY commit
                                fired DURING the write; everything in C0 except
                                LF, plus DEL + C1, is refused at the ONE gated
                                seam. LF stays ALLOWED — paste-shaped multi-line
                                payloads are a first-class flow; \t and \r are
                                refused, a pasted tab-indented snippet being the
                                disclosed cost) + is_bare_slash_payload (the
...
- **Generic decision-prompt detection (Stage B1, display-only, flag-gated) — TWO LEGS: FOOTERED + FOOTERLESS (GH #52)** — a THIRD `UIPattern` in `terminal_parser.UI_PATTERNS`, `Decision`, ordered **LAST** (after Permission/Workflow, so first-match-wins never lets it steal any NAMED pane — AUQ/EPM/Settings/RestoreCheckpoint/Permission/Workflow keep their exact slots). It surfaces GENERIC titled numbered-option confirmation prompts that no named pattern covers (the "Switch model?" confirmation, the folder-trust prompt, and peers). `parse_generic_decision` runs TWO legs in order: the **FOOTERED leg** (`_parse_footered_decision`, byte-identical to the pre-GH#52 shape, stamps `_meta["decision_variant"]="footered"`), then — when the footered leg returns None for ANY reason — the **FOOTERLESS leg** (`_parse_footerless_decision`, stamps `"footerless"`). **FOOTERED leg (pre-2.1.207 `Switch model?`, folder-trust):** loose anchors = a numbered-option TOP + a confirmation footer bottom that MUST carry `Enter to (confirm|continue)` (`_RE_DECISION_FOOTER`, DELIBERATELY excluding `Enter to select`). **Requiring the affirmative-commit `Enter to (confirm|continue)` component (Codex P2 fold) STRUCTURALLY closes the verb-drift veto bypass FOR THE FOOTERED LEG:** a permission gate whose verb is outside `parse_permission_prompt`'s whitelist (`Do you want to open …?`) has an `Esc to cancel · Tab to amend` footer with no `Enter to confirm` line, so it never matches Decision's footer at all. Strict validator: (1) bottom-most confirmation footer; (2) `_only_chrome_below` True; (3) `_gate_options_above` → ≥2 contiguous numbered options AND a resolved live `❯` cursor; (4) **the Permission/Workflow VETO (Hermes P2-4), KEPT as defense-in-depth** — a real permission/workflow gate is NEVER re-surfaced even when `CC_TELEGRAM_PERMISSION_PROMPTS` filtered it out. **FOOTERLESS leg (CC 2.1.207 `Switch model?`, which ends at its last option with NO footer under a full-width `▔` modal rule):** the footered leg's footer requirement structurally EXCLUDES it, so the footerless leg's exclusion rests on DIFFERENT structural means — {STRICT BLANK-BELOW + RULE-ADJACENT TITLE + NAMED-ANCHOR VETOES + verb-agnostic question veto + ATTACHED-FOOTER veto}: (1) a TERMINAL contiguous 1..N numbered block (`_terminal_numbered_option_block`) anchored at the pane's last non-blank line (blank-below to pane end by construction) with a resolved live `❯` cursor; (2) a REQUIRED title sitting IMMEDIATELY below a full-width `▔` rule (`_is_modal_rule`, ≥40 U+2594; `_decision_prompt_block_top` now treats `▔` as a BLOCK TERMINATOR, then the neighbour above the resolved title must BE that rule — a blank line or a `─` rule between them fails, title-less fails; DISCLOSED: the resolved title is the TOP line of the pre-option block, so a PROSE line directly under the rule becomes the title — byte-structurally the same class as the genuine title+subtitle modal (gap3-03 has two contiguous meaningful lines under its rule), refusable only by a content heuristic the generic detector forbids; the cost is a wrong TITLE STRING on a display-only card, never a dispatch); (3) the NAMED-SURFACE vetoes over the PRE-OPTION region ONLY — a `(id, anchor_index, regex.pattern, regex.flags)`-keyed classification map (`_DECISION_ANCHOR_CLASSIFICATION`) marks every `UI_PATTERNS` top anchor `vetoed | excluded(reason)` (each `UIPattern` gains a stable unique `id`; plain-AUQ's + Decision's own generic numbered-option shape are `excluded`, so they don't self-reject; EPM/Settings/RestoreCheckpoint/the checkbox-AUQ anchors are `vetoed`), a tie test asserts EACH anchor is classified EXACTLY once, plus — checked AFTER the classified anchors, as the fallback for NOVEL verbs — the verb-agnostic `^\s*Do you want to\b.*\?` and the strict `parse_permission_prompt`/`parse_workflow_approval`; (4) the ATTACHED-FOOTER veto over the rule→terminal-options region (`_is_attached_footer_veto_line`: strict Decision footer / `·`-key-hint or bounded-single-key hint row / `ctrl+<x>` hint / inline-`(esc)` permission option — Decision's own numbered rows excluded) refuses a strict footer between title and options (a malformed footered mid-redraw frame). **The strict-footerless PREFLIGHT (`extract_interactive_content`, flag-gated on `CC_TELEGRAM_DECISION_CARDS`, so flag-OFF is BYTE-IDENTICAL):** ordering Decision LAST is not enough — a stale COMPLETE AUQ/EPM/Settings surface in the 500-line scrollback ABOVE a live footerless modal matches loosely (those patterns carry no strict validator), so the preflight runs `_parse_footerless_decision` BEFORE accepting any loose validator-less named match; a successful strict footerless parse proves the pane's TERMINAL content is the footerless modal (blank-below + rule-adjacent title + vetoes), and the bottom-most-is-live rule makes any named loose match STALE scrollback above it. A LIVE named surface can never lose the race (while it owns the pane bottom, the footerless parser refuses). **Provenance (`_meta["decision_variant"]`) is POSITIVE authorization:** `decision_prompt_fingerprint` folds the variant into the canonical (footered vs footerless of identical text differ), and derives the footerless option block from the excerpt END (no footer) so a cursor move never churns the fingerprint. **Accepted DROPPED-FOOTER-REDRAW residual (narrowed, not closed):** an answered plain-numbered AUQ whose footer was dropped mid-redraw, rendered `▔`-adjacent, is indistinguishable on the pane from a footerless Decision and DOES footerless-parse (display-only, fail-dark, self-heals on the next complete frame) — the plain-AUQ `excluded(generic-option-shape)` residual, pinned KNOWN-accepted. `current_question_title` = the TOP meaningful line of the prompt block above the options; `pane_excerpt` spans title→last option. Flag-ON by default; display-only in B1. **Detector kill-switch:** the LOCAL parser flag `CC_TELEGRAM_DECISION_CARDS` (`reset_for_tests` resets BOTH flags); `_active_ui_patterns()` filters `Decision` out when OFF AND the preflight is skipped → a flag-OFF deploy adds ZERO new detection. **Render:** `Decision` rides `interactive_ui._GATE_RENDER_NAMES` (display-only card + manual ↑/↓/⏎/Esc nav keyboard); the pane-confirmed `mark_interactive_pending` promotion is UI-name-agnostic, so a live footerless Decision flips the route to "🔔 Waiting on you" (typing off). **Dispatch stays OFF for the footerless variant (Stage B2):** `_build_decision_pick_rows` + `decision_token.identify_family` + the `_dispatch_decision_pane_locked` pre-commit gate + `_classify_decision_advance`'s confirm-side ALL require `decision_variant == "footered"` — a footerless / variant-less form mints NO `dcp:` buttons and never commits. The confirm-side records `dispatched` ONLY when the pane shows a DIFFERENT proven-FOOTERED form OR NO DECISION RESIDUE (`has_decision_residue` — a strict footer line OR a terminal numbered block; a still-standing block IS residue even when unparsed), closing the r5 P1 false-advance where a footered mint re-parsed footerless or a `─`-ruled footer-dropped folder-trust frame extract-None'd. Pull-only; no observer (c313657 forbidden).
...
- **AskUserQuestion option previews (GH #54)** — CC ≥2.1.197 AUQ options can carry a `preview` (multi-line ASCII-art mockup). The TUI renders single-select previews SIDE-BY-SIDE (numbered options left, the FOCUSED option's `preview` in a right box-drawing panel that SWITCHES on every cursor move) and renders multi-select previews NOT AT ALL — so the 📋 details card (W3) is the only place a multi-select preview is seen. **Capture spine (W1):** `tmux_manager.capture_pane_pair` captures WITH ANSI + derives the plain twin via `terminal_parser.normalize_capture(raw_ansi) -> PaneCapture(plain, ansi) | None` — strips every ECMA-48 string family (CSI + OSC/DCS/SOS/PM/APC through BEL/ST + ESC-single + tmux `-e` pad), REJECTS on an unknown control seq, permits exactly ONE plain re-capture per observation (WARNING-logged, the sole one-capture exception); region-equality acceptance over the fixture pairs. Four ANSI seams: `handle_interactive_ui` (BOTH phases — visible liveness probe + 500-line scrollback parse, each its own pair), the `status_polling` render-hash path (an SGR-only cursor move repaints), `pick_token.validate_and_consume`/`recover_and_consume`, and `_dispatch_pick`; `resolve_ask_form`/`parse_ask_user_question` gain a keyword-only `ansi_text`. **Parse (W1):** a footer-anchored region prepass (POSITIVE preview-footer chrome, anchored to the bottom-most-live picker) REPLACES the ordinary 0-option walk; a display-CELL panel-boundary (shared `wcswidth`, chrome rows never vote) truncates every option row so PANEL TEXT NEVER reaches the form / `_pane_fingerprint` / render signature (the hard exclusion invariant, IDENTICAL fingerprint across `cursor1`/`cursor2`). Completeness = contiguous 1..N + positive preview chrome (the numbered-affordance completeness proof has no preview counterpart); trusted minting is DECLINED for every pane-only MULTI-question preview lacking the `questions` matrix (it could nav+Enter but never confirm). **Two-tier cursor by LAYOUT STATE, not version (W1.3):** tier 1 = `❯` glyph when nothing wraps; tier 2 = SGR bold+fg-153 over LOGICAL options (a wrapping label drops the chevron on BOTH versions) — exactly one consistently-bold option ⇒ cursor, else NO cursor; `_overlay_cursor_and_selection` NEVER synthesizes a cursor on a `_meta["layout"]=="preview"` form (no proven cursor ⇒ dispatch bails `not_advanced`), a TUI-drift audit surface. **Wrap-canonical matching (W1.2):** `AskOption.wrap_canonical` (no-space fragment join, `compare=False`) + `terminal_parser.label_matches_authority` accepts EITHER the space-joined OR the space-stripped form against the authority (both wrap kinds match without guessing the space), used across `_record_consistent_with_pane` / `_loose_label_match` / `_strong_match` / `_infer_current_tab_idx` / partial-context recovery. **W2 rescue-gate parity:** `resolve_auq_source_for_render` rescues on `pane_form is None OR not pane_form.options` (the `no_pane_form` definition — mint/validate parity), and W2 ellipsis-normalizes damaged pane fragments so a truncation-shred can never contradict the side file into a false non-rescue. **W3 ctx-card previews:** `interactive_ui._format_auq_context_message` (DICT path only — the pane-form path is untouched) appends each `preview` as a fenced MONOSPACE code block; RAW Markdown (telegramify converts ONCE — `maybe_upgrade`'s edit path flipped `plain=True`→`plain=False`), `_sanitize_preview` strips `\x02` sentinel bytes + defangs 3+-backtick runs with U+200B, and chunking is RENDERED-COST-BOUNDED (r1 P2-2 — MarkdownV2 escaping expands ~2×, so a raw-budget split can overshoot 4096): `_build_ctx_parts` (BOTH ctx seams) refits any part whose ACTUAL `convert_markdown` cost exceeds 4096 via `split_message` at an expansion-ratio-scaled raw budget — the recap lane's rendered-cost discipline, `split_message` staying the ONE fence balancer (more chunks, never truncation). `build_form_from_tool_input` still ignores `preview`, so the form fingerprint is UNCHANGED by W3. **W5 copy honesty (r1→r4 — the card copy is a DRY-RUN of the executor):** three parallel-predicate rounds each left a reproduced over-advertising state (r1 missed `options_complete`/multi-Q; r2 missed cursor + anchor; r3 consumed the resolver-MERGED form — which restores options, forces completeness, synthesizes cursors — checked anchor EXISTENCE not anchor–pane AGREEMENT, and omitted the stranded-draft brake), so the executor's ENTIRE pre-keystroke phase is now ONE callable, `free_text.plan_pre_keystroke` (flag + brake peek + Claude proof + `plan_from_pane` incl. `derive_identity`'s mandatory-anchor agreement + license), CONSUMED by `try_answer`'s `_answer_locked` (with two sanctioned idempotent duplicates — the early brake check + the r5 P2 early `not_claude` decline on the in-lock probe BEFORE `_observe`, preserving probe-first ordering so a non-Claude pane declines with zero captures instead of propagating a capture raise) AND dry-run by `advertises_free_text(surface, version=…, window_id=…, pane_text=…, ansi_pane=…)` on the render seam's RAW captured pane pair + the executor's own `read_surface_anchor` (the merged form no longer decides the copy — it only renders the body). The three notices + `card_hint` route through it; honest-at-render (the executor's gates stay the send-time authority); pinned by fresh-`side_file_ok` scenarios (partial/cursorless raw pane, mismatched side file, braked window ⇒ nav-only; agreeing complete pane ⇒ text) + delegation pins proving invocation-unity, decline-honored and success-consumed (not gate-absence — a review invariant). No route_runtime change, no new state file, no observer (c313657). Full contract in `message-handling.md`.
