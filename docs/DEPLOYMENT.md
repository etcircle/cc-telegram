...
Useful Telegram commands: `/history`, `/screenshot`, `/esc`, `/settings`, `/dashboard [pin]`, `/kill`, `/unbind`, `/usage`, `/cost` (intercepted bot-side: on an idle session captured, answered as a message, auto-dismissed; on a busy session — or one with an unsent terminal draft — no keystroke is injected and the bot replies with a bridge-side snapshot: context usage + the last full overlay it cached, with an age label). Forwarded Claude commands: `/clear`, `/compact`, `/model`, `/effort`. Closing/deleting the topic kills the tmux window and unbinds it.
...
| **Voice note → “transcription timed out”** | The duration-scaled end-to-end attempt budget expired (2× Telegram duration, clamped to 120–600s); the provider may have completed an upload whose result did not return | Resend the note or split it into shorter segments; inspect the content-free `voice transcription received` / `failed classification=timeout` INFO log lines |
| **Voice note fails once without repeated API calls** | Read/write timeout, ambiguous transport failure, non-429 4xx, 5xx, empty result, or unusable 429 `Retry-After`; these are intentionally not auto-retried because the paid request may have completed | Resend manually after checking the provider. Automatic retry is limited to connect failures and HTTP 429 with absent or integer 0–10s `Retry-After` |
| **Voice transcription reached Claude but no 🎤 echo appeared** | Telegram echo delivery failed after the aggregator accepted the turn | The turn is still delivered; look for the `voice transcription echo failed` WARNING log line |
