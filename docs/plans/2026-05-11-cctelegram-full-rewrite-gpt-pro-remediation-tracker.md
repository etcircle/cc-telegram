...
- 2026-05-11: Item 4 implemented by Hermes agent via loud `doctor --migrate` refusal when both legacy and target dirs exist plus README migration reorder, parent verification passed (`9 passed`, ruff clean, `pyright src/cctelegram/doctor.py` clean), independent Hermes reviewer returned PASS. Ready to freeze/commit.
