...
async def topic_edit_reply_markup(
    ...
    *,
    ...
    Used by the multi-tab AskUserQuestion state machine to move pick
    buttons between per-tab cards without re-editing the body — Telegram
    diffs ``reply_markup`` independently of text, so this is the right
    API for "the body is unchanged, only the keyboard moves." Calling
    ``edit_message_text`` with the same body would raise MESSAGE_NOT_MODIFIED
    when only the markup actually changed, and pollute the failure log.

    ``reply_markup=None`` strips the keyboard. MESSAGE_NOT_MODIFIED on
    this endpoint is benign (Telegram says "you asked for the same
    markup that's already there"); we map it to OK so callers can treat
    ...
    try:
        ...
        outcome = _classify_bad_request(exc)
        # Treat "already had this markup" as a benign success — the post-
        # condition (markup matches what we wanted) is satisfied.
        if outcome is TopicSendOutcome.MESSAGE_NOT_MODIFIED:
            ...
            return TopicSendOutcome.OK
        _log_topic_outcome(
            op, user_id, chat_id, thread_id, window_id, outcome, "edit_markup", exc
        )
        return outcome
